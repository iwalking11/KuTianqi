package com.gaopinwei.kutianqi.request.api;
...
public class ShowApiRequest     {
	...
	public ShowApiRequest( String url,String appid,String appSecret    ) {
		...
		params.put(com.show.api.Constants.SHOWAPI_APPID,appid);
		textMap.put(com.show.api.Constants.SHOWAPI_APPID,appid);
	}
	...
	private void addSign() throws  Exception{
		boolean ismd5=true;
		if(textMap.get(com.show.api.Constants.SHOWAPI_APPID)==null)throw new Exception( errorMsg(com.show.api.Constants.SHOWAPI_APPID+"不得为空!"));
		String signmethod=textMap.get(com.show.api.Constants.SHOWAPI_SIGN_METHOD);
		...
		if(textMap.get(com.show.api.Constants.SHOWAPI_TIMESTAMP)==null){
			SimpleDateFormat df=new SimpleDateFormat(com.show.api.Constants.DATE_TIME_FORMAT);
			String timestamp= df.format(new Date());
			params.put(com.show.api.Constants.SHOWAPI_TIMESTAMP,timestamp); //放时间
			textMap.put(com.show.api.Constants.SHOWAPI_TIMESTAMP,timestamp); //放时间
		}
		String sign;
		if (ismd5) {
			sign= com.show.api.ShowApiUtils.signRequest(textMap, appSecret, false);
		} else {
			sign= com.show.api.ShowApiUtils.signRequest(textMap, appSecret, true);
		}
